...
# Load university coordinates once
universities_df = pd.read_csv("universities.csv")
...
def student_score():
    ...
    try:
        prop_location = (lat, lon)

        # Compute distances to all universities
        universities_df['distance_km'] = universities_df.apply(
            lambda row: geodesic(prop_location, (row['lat'], row['lon'])).km,
            axis=1
        )

        # Find nearest
        nearest = universities_df.nsmallest(1, 'distance_km').iloc[0]

        # Basic score (you can evolve this later)
        score = max(0, 100 - nearest['distance_km'])
